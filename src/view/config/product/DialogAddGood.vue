...
// 获取详情
const getDetail = (id) => {
  axiosInstance.get(`/indexConfigs/${id}`).then(res => {
    state.ruleForm = {
      name: res.data.configName,
      id: res.data.goodsId,
      link: res.data.redirectUrl,
      sort: res.data.configRank
    }
  })
}
// 开启弹窗
...
const submitForm = () => {
  formRef.value.validate((valid) => {
    if (valid) {
      ...
      if (props.type == 'add') {
        axiosInstance.post('/indexConfigs', {
        ...
        axiosInstance.put('/indexConfigs', {
          configId: state.id,
          configType: props.configType || 3,
          configName: state.ruleForm.name,
          redirectUrl: state.ruleForm.link,
